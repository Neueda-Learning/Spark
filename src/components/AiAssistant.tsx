...
function detectQuestionLanguage(text: string): 'zh' | 'en' {
  const cjkCount = Array.from(text).filter((ch) => /[\u4e00-\u9fff]/.test(ch)).length;
  const latinCount = Array.from(text).filter((ch) => /[A-Za-z]/.test(ch)).length;
  return latinCount > cjkCount && latinCount > 3 ? 'en' : 'zh';
}
...
function AiAssistant() {
  const [messages, setMessages] = useState<AssistantMessage[]>([
    {
      role: 'assistant',
      content: 'Hello, I am your investment assistant. You can ask me about portfolio risk, rebalancing ideas, and position-building directions.',
    },
  ]);
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    const locale = detectQuestionLanguage(text);
    const userMessage: AssistantMessage = { role: 'user', content: text, locale };
    const placeholder: AssistantMessage = { role: 'assistant', content: '', locale };
    ...
    try {
      ...
      setMessagesSync((prev) => {
        ...
        const last = next[next.length - 1];
        if (last && last.role === 'assistant') {
          last.content = parsed.text.trim() || 'The model returned no content this time.';
      ...
      setError('AI assistant is temporarily unavailable. Please check backend model settings and try again.');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4 md:p-6 h-[82vh] min-h-[640px] flex flex-col">
      <div className="mb-2">
        <h2 className="text-lg font-semibold text-gray-900">Invest Assistant</h2>
        <p className="text-xs text-gray-500 mt-0.5">The following answers are generated by a large language model and are for reference only.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto border border-gray-200 rounded-lg p-3 space-y-3 bg-slate-50">
        {renderedMessages.map((m, idx) => {
          ...
          const lineTitleFallback = locale === 'zh' ? '当前持仓预测盈亏' : 'Current Portfolio Forecast Return';
          const currentLegendFallback = locale === 'zh' ? '当前组合' : 'Current Portfolio';
          const recommendedLegendFallback = locale === 'zh' ? '建议组合' : 'Recommended Portfolio';
          const pieTitleFallback = locale === 'zh' ? '建议持仓占比' : 'Recommended Allocation';
          const barTitleFallback = locale === 'zh' ? '收益驱动因子' : 'Expected Return Drivers';
                ...
                <div className="mr-auto max-w-[92%] grid grid-cols-1 lg:grid-cols-2 gap-3">
                  {selectedCharts.includes('line') && lineData.length > 1 && (
                    <div className="bg-white border border-gray-200 rounded-lg p-3 lg:col-span-2">
                      <p className="text-xs text-gray-600 mb-2">
                        {m.charts?.current_portfolio_forecast?.title || lineTitleFallback}
                      </p>
                      <div className="h-44">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={lineData}>
                            ...
                            <Legend />
                            <Line type="monotone" dataKey="current" name={currentLegendFallback} stroke="#ef4444" strokeWidth={2} dot={false} />
                            {showRecommendedLine && (
                              <Line type="monotone" dataKey="recommended" name={recommendedLegendFallback} stroke="#2563eb" strokeWidth={2} dot={false} />
                            )}
                          </LineChart>
                        </ResponsiveContainer>
                      </div>
                  ...
                  {selectedCharts.includes('pie') && pieData.length > 0 && (
                    <div className="bg-white border border-gray-200 rounded-lg p-3">
                      <p className="text-xs text-gray-600 mb-2">
                        {m.charts?.recommended_allocation?.title || pieTitleFallback}
                      </p>
                      <div className="h-40">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                  ...
                  {selectedCharts.includes('bar') && driverData.length > 0 && (
                    <div className="bg-white border border-gray-200 rounded-lg p-3">
                      <p className="text-xs text-gray-600 mb-2">
                        {m.charts?.expected_drivers?.title || barTitleFallback}
                      </p>
                      <div className="h-40">
                        <ResponsiveContainer width="100%" height="100%">
        ...
        {sending && <div className="text-xs text-gray-500">Assistant is thinking...</div>}
      </div>
      ...
      <form onSubmit={onSubmit} className="mt-3 flex gap-2">
        <input
          ...
          placeholder="For example: please suggest a rebalance plan based on my current holdings"
